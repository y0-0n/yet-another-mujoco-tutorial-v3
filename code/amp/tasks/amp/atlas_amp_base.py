...
# 85
NUM_OBS = 1+6+3+3+30+30+12# 1 + 6 + 3 + 3 + 70 + 37 + 12 # [(root_h(z-height):1, root_rot:6, root_vel:3, root_ang_vel:3, dof_pos, dof_vel, key_body_pos]
NUM_ACTIONS = 30    # the number of tendon actuations
...
class AtlasAMPBase(VecTask):
    ...
    def _create_ray_envs(self):
        self.standard_env = MuJoCoParserClass(name='Atlas',rel_xml_path=self.asset_file, VERBOSE=False,USE_MUJOCO_VIEWER=False)
        ...
        power_scale = 0.3
        # Ray parallelization
        self.mujoco_envs = [MuJoCoParserClassRay.remote(name='Atlas Ray',
        ...
        self.test_envs = [MuJoCoParserClassRay.remote(name='Atlas Test',
                                                        rel_xml_path=self.asset_file,
                                                        VERBOSE=False,
                                                        USE_MUJOCO_VIEWER=(not self.headless),
                                                        device=self.device,
        ...
        if self.headless == False:
            for i, env in enumerate(self.mujoco_envs):
                env.init_viewer.remote(viewer_title='Atlas Ray'+str(i),viewer_width=1200,viewer_height=800,
        ...
        self.right_foot_idx = self.standard_env.model.body('r_foot').id # NOTE: Different from Isaac index
        self.left_foot_idx = self.standard_env.model.body('l_foot').id # NOTE: Different from Isaac index

        self.torso_index = self.standard_env.model.body('pelvis').id # NOTE: Different from Isaac index
    ...
    def _build_key_body_ids_tensor(self):
        ...
        # body_ids = to_torch([6, 9, 12, 15], device=self.device, dtype=torch.long)
        return body_ids

    def _build_contact_body_ids_tensor(self):
        body_ids = self.standard_env.get_body_ids(self._contact_bodies)
        body_ids = to_torch(body_ids, device=self.device, dtype=torch.long)
        # body_ids = to_torch([12, 15], device=self.device, dtype=torch.long)
        return body_ids
...
def compute_humanoid_observations(root_states, dof_pos, dof_vel, key_body_pos, local_root_obs):
    ...
    obs = torch.cat((root_h, root_rot_obs, local_root_vel, local_root_ang_vel, dof_pos, dof_vel, flat_local_key_pos), dim=-1)
    return obs
