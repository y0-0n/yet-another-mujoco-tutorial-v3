...
NUM_OBS = 1 + 6 + 3 + 3 + 32 + 20 + 6 # [(root_h(z-height):1, root_rot:6, root_vel:3, root_ang_vel:3, dof_pos, dof_vel, key_body_pos]
NUM_ACTIONS = 80    # the number of tendon actuations
NUM_ACTORS_PER_ENVS = 1 # Added from JTM, 2 actors per envs

KEY_BODY_NAMES = ["talus_r", "talus_l"]

class MyoSuiteAMPBase(VecTask):
    ...
    def _create_ray_envs(self):
        self.standard_env = MuJoCoParserClass(name='MyoSuite',rel_xml_path=self.asset_file, VERBOSE=False,USE_MUJOCO_VIEWER=False)
        ...
        # Ray parallelization
        self.mujoco_envs = [MuJoCoParserClassRay.remote(name='MyoSuite Ray',
        ...
        for i, env in enumerate(self.mujoco_envs):
            if self.headless == False:
                env.init_viewer.remote(viewer_title='MyoSuite Ray'+str(i),viewer_width=1200,viewer_height=800,
        ...
        self.right_foot_idx = self.standard_env.model.body('talus_r').id # NOTE: Different from Isaac index
        self.left_foot_idx = self.standard_env.model.body('talus_l').id # NOTE: Different from Isaac index
